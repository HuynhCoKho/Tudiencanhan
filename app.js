...
function loadLocal(){ customEntries = JSON.parse(localStorage.getItem(CUSTOM_KEY) || '[]'); deletedIds = new Set(JSON.parse(localStorage.getItem(DELETED_KEY) || '[]')); }
function saveLocal(){ localStorage.setItem(CUSTOM_KEY, JSON.stringify(customEntries)); localStorage.setItem(DELETED_KEY, JSON.stringify([...deletedIds])); }
...
async function importJson(files){
  ...
  for (const file of files) {
    ...
    entries.map(cleanEntry).forEach(e => {
      customEntries.unshift({ ...e, id: makeId(), sourceName: file.name });
      added += 1;
    });
  }
  saveLocal();
...
function bind(){
  ...
  $('importFile').onchange = e => importJson([...e.target.files]).catch(err => toast('Không nhập được file: ' + err.message));
...
async function init(){
  ...
  try {
    ...
    baseEntries = (Array.isArray(data) ? data : data.entries || []).map(cleanEntry);
    toast(`Đã tải ${baseEntries.length} mục từ.`);
  } catch (error) {
